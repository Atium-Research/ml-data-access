...
def test_windowed_loaders_respect_the_window(db):
    sessions = ml_data.load_sessions(db, dt.date(2025, 1, 1), None)
    assert all(day >= dt.date(2025, 1, 1) for day in sessions) and len(sessions) == 3
    universe_df = ml_data.load_universe(db, dt.date(2024, 12, 30), dt.date(2024, 12, 31))
    assert universe_df["date"].n_unique() == 2 and set(universe_df["symbol"]) == {"AAPL", "BRKB"}
    assert ml_data.available_years("universe") == [2024, 2025]


def test_chain_loader_opens_one_symbol_and_filters_the_window(db):
    chain_df = ml_data.load_option_greeks(db, "aapl", dt.date(2025, 1, 1), dt.date(2025, 1, 2))
    assert set(chain_df["symbol"]) == {"AAPL"}
    assert chain_df["date"].min() >= dt.date(2025, 1, 1)
    assert ml_data.available_symbols("option_greeks", 2025) == ["AAPL", "BRKB"]
    with pytest.raises(FileNotFoundError):
        ml_data.load_option_greeks(db, "MSFT")
    with pytest.raises(FileNotFoundError):
        ml_data.load_open_interest(db, "AAPL")


def test_screens(db):
    usable_df = ml_data.usable_symbol_years(db)
    ...
    assert ml_data.in_universe(db, panel_df)["symbol"].to_list() == ["AAPL"]


def test_signals_loader_picks_one_signal(db):
    scores_df = ml_data.load_signals(db, "momentum")
